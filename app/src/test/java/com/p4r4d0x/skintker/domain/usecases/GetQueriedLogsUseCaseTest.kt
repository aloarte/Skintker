...
class GetQueriedLogsUseCaseTest : KoinBaseTest(testRepositoriesModule) {
    ...
    fun `test get queried logs by irritation intensity`() {
        val logList = listOf(
            DailyLogBO(Date(), IrritationBO(8, emptyList()), foodList = emptyList()),
            DailyLogBO(Date(), IrritationBO(9, emptyList()), foodList = emptyList())
        )
        ...
        val expectedPossibleCauses = PossibleCausesBO(
            ...
            stressCause = PossibleCausesBO.StressCauseBO(averageLevel = -1, possibleCause = false),
            travelCause = PossibleCausesBO.TravelCauseBO(city = null, possibleCause = false),
            weatherCause = Pair(
